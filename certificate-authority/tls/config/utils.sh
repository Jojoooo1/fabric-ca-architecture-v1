createNodeIdentity() {
  ...
  if [ -z "$NODE_TYPE" ] || [ -z "$NODE_NUMBER" ]; then
    echo "Please pass a NODE_TYPE/NODE_NUMBER to createNodeIdentity()"
    exit 1
  fi

  if [ $NODE_TYPE != "peer" ] && [ $NODE_TYPE != "orderer" ]; then
    echo "Please pass a correct NODE_TYPE value to createNodeIdentity()"
    exit 1
  fi

  for ((j = 0; j < $NODE_NUMBER; j++)); do # loop every peer
    IDENTITY_NAME="$NODE_TYPE$j.$ORG_FULL_NAME"
    ORG_ICA_IDENTITY_IDENTITY_MSP_DIR="$ORG_ICA_IDENTITY_DIR/${NODE_TYPE}s/$IDENTITY_NAME/msp"
    ORG_ICA_IDENTITY_IDENTITY_TLS_DIR="$ORG_ICA_IDENTITY_DIR/${NODE_TYPE}s/$IDENTITY_NAME/tls"
...
generateIntermediateCAIdentityTLS() {
  ...
  for i in ${!ORGANIZATION_NAME[@]}; do
    ...
    PEER_NUMBER=${ORGANIZATION_PEER_NUMBER[$i]}
    createNodeIdentity "peer" $PEER_NUMBER

    ORDERER_NUMBER=${ORGANIZATION_ORDERER_NUMBER[$i]}
    createNodeIdentity "orderer" $ORDERER_NUMBER
