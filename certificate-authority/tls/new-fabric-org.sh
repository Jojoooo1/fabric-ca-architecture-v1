...
generateIntermediateCAIdentity() {
  ...
  for i in ${!ORGANIZATION_NAME[@]}; do
    ...
    for ((j = 0; j < ${ORGANIZATION_PEER_NUMBER[$i]}; j++)); do # loop every peer
      ...
      cp $ORG_ICA_DIR/tls-chain.$ORG_FULL_NAME-cert.pem $ORG_ICA_IDENTITY_IDENTITY_TLS_DIR/ca.crt
    ...
    if [ ! -d $TLS_CA ]; then
      ...
      # **!!** RENAME CHAIN CERT TO ca.cert to KEEP SAME FABRIC FORMAT FOR ALL PEER **!!**
      cp $ORG_ICA_DIR/tls-chain.$ORG_FULL_NAME-cert.pem $ORG_ICA_IDENTITY_CA_DIR/ca.crt
      # Copy RCA/ICA/CHAIN cert in PEER_FOLDER
    ...
    cp $ORG_RCA_DIR/certs/tls-rca.$ORG_FULL_NAME-cert.pem $ORG_ICA_IDENTITY_ORG_MSP_DIR/tlscacerts
    cp $ORG_ICA_DIR/certs/tls-ica.$ORG_FULL_NAME-cert.pem $ORG_ICA_IDENTITY_ORG_MSP_DIR/tlsintermediatecerts
