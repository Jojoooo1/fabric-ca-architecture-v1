...
ORGANIZATION_MSPID=(${ORGANIZATION_NAME[0]}MSP ${ORGANIZATION_NAME[1]}MSP)
# ORGANIZATION_DOMAIN=(${ORGANIZATION_NAME[0],,}.${DOMAIN}) # ORGANIZATION_NAME in minuscula
ORGANIZATION_DOMAIN=(${ORGANIZATION_NAME[0],,}.${DOMAIN} ${ORGANIZATION_NAME[1],,}.${DOMAIN}) # ORGANIZATION_NAME in minuscula

ORGANIZATION_PEER_NUMBER=(2 2)              # Template count in crypto-config && modify docker-compose
